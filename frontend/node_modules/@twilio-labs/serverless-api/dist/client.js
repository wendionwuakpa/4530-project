...
function createGotClient(config) {
    ...
    if ('accountSid' in config) {
        ...
        util_1.deprecate(() => { }, '`accountSid` and `authToken` client config is deprecated, please use `username` and `password` instead.')();
    }
    else {
    ...
    const client = got_1.default.extend({
        prefixUrl: api_client_1.getApiUrl(config),
        responseType: 'json',
        username: username,
        password: password,
        headers: {
...
class TwilioServerlessApiClient extends events_1.default.EventEmitter {
    constructor(config) {
        ...
        this.limit = p_limit_1.default(config.concurrency || http_config_1.CONCURRENCY);
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (types === 'services' ||
                    ...
                    const services = yield services_1.listServices(this);
                    return { services };
                }
                if (typeof serviceSid === 'undefined' &&
                    typeof serviceName !== 'undefined') {
                    serviceSid = yield services_1.findServiceSid(serviceName, this);
                }
                if (typeof serviceSid === 'undefined') {
                ...
                for (const type of types) {
                    try {
                        if (type === 'environments') {
                            result.environments = yield environments_1.listEnvironments(serviceSid, this);
                        }
                        if (type === 'builds') {
                            result.builds = yield builds_1.listBuilds(serviceSid, this);
                        }
                        if (typeof environmentSid === 'string') {
                            if (!environments_1.isEnvironmentSid(environmentSid)) {
                                const environment = yield environments_1.getEnvironmentFromSuffix(environmentSid, serviceSid, this);
                                environmentSid = environment.sid;
                                currentBuildSidForEnv = environment.build_sid;
                            }
                            else if (!currentBuildSidForEnv) {
                                const environment = yield environments_1.getEnvironment(environmentSid, serviceSid, this);
                                currentBuildSidForEnv = environment.build_sid;
                            }
                            if (type === 'functions' || type === 'assets') {
                                if (!currentBuild) {
                                    currentBuild = yield builds_1.getBuild(currentBuildSidForEnv, serviceSid, this);
                                }
                                if (type === 'functions') {
                                    result.functions = {
                                        environmentSid,
                            ...
                            if (type === 'variables') {
                                result.variables = {
                                    entries: yield variables_1.listVariablesForEnvironment(environmentSid, serviceSid, this),
                                    environmentSid,
                                };
                            }
                        }
                ...
                error_1.convertApiErrorsAndThrow(err);
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!environments_1.isEnvironmentSid(environment)) {
                    const environmentResource = yield environments_1.getEnvironmentFromSuffix(environment, serviceSid, this);
                    environment = environmentResource.sid;
                }
                if (filterByFunction && !functions_1.isFunctionSid(filterByFunction)) {
                    const availableFunctions = yield functions_1.listFunctionResources(serviceSid, this);
    ...
    getLogs(logsConfig) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                return logs_1.listOnePageLogResources(environment, serviceSid, this, {
                    pageSize: 50,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!environments_1.isEnvironmentSid(targetEnvironment)) {
                    try {
                        const environment = yield environments_1.getEnvironmentFromSuffix(targetEnvironment, serviceSid, this);
                        targetEnvironment = environment.sid;
                    }
                    catch (err) {
                        if (activateConfig.force || activateConfig.createEnvironment) {
                            const environment = yield environments_1.createEnvironmentFromSuffix(targetEnvironment, serviceSid, this);
                            targetEnvironment = environment.sid;
                        }
                        else {
                            throw err;
                ...
                if (!buildSid && sourceEnvironment) {
                    ...
                    if (!environments_1.isEnvironmentSid(sourceEnvironment)) {
                        currentEnv = yield environments_1.getEnvironmentFromSuffix(sourceEnvironment, serviceSid, this);
                    }
                    else {
                        currentEnv = yield environments_1.getEnvironment(sourceEnvironment, serviceSid, this);
                    }
                ...
                yield variables_1.setEnvironmentVariables(env, targetEnvironment, serviceSid, this);
                const { domain_name } = yield environments_1.getEnvironment(targetEnvironment, serviceSid, this);
                yield builds_1.activateBuild(buildSid, targetEnvironment, serviceSid, this);
                return {
                    serviceSid,
                    buildSid,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!serviceSid) {
                    ...
                    try {
                        serviceSid = yield services_1.createService(config.serviceName, this);
                    }
                    catch (err) {
                        const alternativeServiceSid = yield services_1.findServiceSid(config.serviceName, this);
                ...
                const environment = yield environments_1.createEnvironmentIfNotExists(config.functionsEnv, serviceSid, this);
                ...
                const functionResources = yield functions_1.getOrCreateFunctionResources(functions, serviceSid, this);
                this.emit('status-update', {
                ...
                const functionVersions = yield Promise.all(functionResources.map((fn) => {
                    return functions_1.uploadFunction(fn, serviceSid, this, this.config);
                }));
                //
                // Assets
                //
                ...
                const assetResources = yield assets_1.getOrCreateAssetResources(assets, serviceSid, this);
                this.emit('status-update', {
                ...
                const assetVersions = yield Promise.all(assetResources.map((asset) => {
                    return assets_1.uploadAsset(asset, serviceSid, this, this.config);
                }));
                this.emit('status-update', {
                ...
                const build = yield builds_1.triggerBuild({ functionVersions, dependencies, assetVersions, runtime }, serviceSid, this);
                yield builds_1.waitForSuccessfulBuild(build.sid, serviceSid, this, this);
                this.emit('status-update', {
                    status: consts_1.DeployStatus.SETTING_VARIABLES,
                    message: 'Setting environment variables',
                });
                yield variables_1.setEnvironmentVariables(config.env, environmentSid, serviceSid, this);
                this.emit('status-update', {
                    status: consts_1.DeployStatus.ACTIVATING_DEPLOYMENT,
                    message: 'Activating deployment',
                });
                yield builds_1.activateBuild(build.sid, environmentSid, serviceSid, this);
                this.emit('status', {
